...
parser = argparse.ArgumentParser(
    description="Evaluate DeepFuse on specific dataset fold"
)
parser.add_argument(
    "--dataset",
    type=str,
    required=True,
    choices=["BF-C2DL-HSC", "BF-C2DL-MuSC"],
    help="Dataset name",
)
parser.add_argument(
    "--fold",
    ...
    required=True,
    choices=[1, 2],
    help="Cross-validation fold (1 or 2)",
)
parser.add_argument(
    "--model", type=str, required=True, help="Path to .h5 model file to evaluate"
)
...
parquet_path = (
    ...
    / dataset_name
    / "whole_image"
    / f"{dataset_name}_split_fold-{fold_num}.parquet"
)

# Competitor input paths (5 raters)
# TEST split uses the OPPOSITE sequence from train/val:
#   Fold 1: test on seq 02
#   Fold 2: test on seq 01
if fold_num == 1:
    input_paths = [
...
log_filename = os.path.join(
    output_dir, f"evaluation_results_{dataset_name}_fold{fold_num}.txt"
)
...
def build_model():
    ...
    model = Model(
        inputs=[x1.input, x2.input, x3.input, x4.input, x5.input], outputs=[output]
    )

    model.compile(
        loss=dice_coef_loss, optimizer=Adam(lr=learning_rate), metrics=[dice_coef]
    )
    model.summary()
    return model
...
def main():
    ...
    for m_path in model_files:
        try:
            ...
            print(
                f"{os.path.relpath(m_path, output_dir):<80} | {'TEST':<10} | {f1_test:.4f}     | {jac_test:.4f}"
            )
            print("-" * 120)
