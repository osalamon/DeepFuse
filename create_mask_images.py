...
def create_train_data(input_path, gt_path):
    ...
    # 2. init of list for collecting processed sub-images (i.e. single object - cell)
    objects_subimages = []

    # 3. Firstly, I define width and height STATICALLY for cropping sub-images around each object
    ...
    for image_name in os.listdir(gt_path):  # Go through all the .tif files in the folder
        if image_name.endswith(".tif"):
            ...
            cur_labels = np.unique(im_gt)
            cur_labels = cur_labels[cur_labels != 0]     # Remove 0 (background) if present # TODO not sure about this line
